...
- **Conexión automática al MCP del issue tracker en Copilot.** El init (`/sooft`) ahora escribe `.vscode/mcp.json` con el server remoto `servicenow` (Streamable HTTP, sin autenticación en su versión actual) y `.vscode/settings.json` con `chat.mcp.autoStart: true` (Experimental). Con esa setting, VS Code **levanta el server solo** al detectar el `mcp.json` —sin el `MCP: List Servers → Start` manual—; la única interacción que queda es aceptar la **confianza** del server la primera vez (control de seguridad de VS Code, por única vez por proyecto). La experiencia del developer queda en 2 pasos —`npx skills add` + `/sooft`— sin tokens ni configuración manual. Ambos archivos se agregan por **merge** (no pisan otras claves/servers) y se escriben **siempre** durante el init (`settings.json` antes que `mcp.json`).
- **Sección "Copilot (VS Code)" en `integrations/mcp/servicenow.md`** — documenta el formato `.vscode/mcp.json` (clave `servers`, server remoto HTTP), aclara que `npx skills add` NO instala el MCP, y deja el patrón con `input` `password` para cuando el server sume autenticación.
- **Plantilla `templates/mcp.json`** — `.vscode/mcp.json` de ejemplo con el server `servicenow`. Sirve para que quien administra el repo del piloto lo pre-commitee y los devs tengan el server activo desde el arranque (sin el clic de *MCP: List Servers → Start* de la primera sesión). El flujo del dev sigue siendo `npx skills add` + `/sooft`; si el archivo ya existe, `/sooft` no lo pisa (merge).
- **Plantilla `templates/mcp-cli.json`** — `.mcp.json` de ejemplo para el **CLI de Copilot** (clave `mcpServers`, server `servicenow` con `"tools":["*"]`). Complementa a `templates/mcp.json` (que es el `.vscode/mcp.json` de VS Code, clave `servers`): son los dos archivos que `/sooft` deja para que el MCP arranque solo en ambos clientes.
...
- **Integración el escáner SAST (SAST)** — `integrations/veracode/`. Validación de seguridad obligatoria antes del PR, complementaria a el análisis estático (calidad) y al linter (estilo). Hallazgos Very High/High bloquean el PR.
...
- `pre-pr-check.sh`: agrega verificación de el escáner SAST y de marcadores `[IA-generated]`.
- `rules/security.md`: SAST con el escáner SAST como validación obligatoria.
- Templates `evidence.md`: secciones de el escáner SAST y de código IA-generated.
...
- **Reglas de Sooft** — architecture, security, testing, documentation, service-now.
