...
- **Hook de sesión con un único algoritmo, igual para cualquier herramienta.** Hasta ahora el único hook determinista de SOOFT (banner + recordatorio de cargar la skill antes de tocar código) existía solo para GitHub Copilot (`assets/sooft.json`, instalado por `internal/sooft-bootstrap.md`); ninguna otra herramienta tenía un hook equivalente, aunque `SKILL.md` §0.1 ya lo mencionaba como pendiente ("en Claude Code se configuran por otro mecanismo; no los instales acá"). Se agrega `skills/sooft/assets/hooks/session-start.yml` como fuente única de verdad del contenido (banner + `additional_context`) y de un `algoritmo_universal` de 4 pasos: reusar un adapter ya verificado de la librería `assets/hooks/adapters/` si existe; si no, construir y verificar uno nuevo solo si el mecanismo de hooks de la herramienta actual se puede confirmar contra su propia documentación (PROHIBIDO inventar el nombre de un evento o el formato de un payload); si no se puede confirmar, no instalar nada — SOOFT funciona igual, la misma regla ya vive en las instrucciones always-on; y si se construyó un adapter nuevo, guardarlo en la librería para que el próximo developer con esa misma herramienta ya lo encuentre resuelto. Copilot y Claude Code no son "las dos soportadas": son dos veces que ya se corrió ese algoritmo. Se implementa el adapter verificado para Claude Code (`assets/hooks/adapters/claude.json`, un bloque `hooks.SessionStart` para mergear en `.claude/settings.json` sin pisar configuración propia del equipo, con su propio `adapters/README.md` documentando el patrón autoextensible). La instalación automática desde `sooft-tech agent install` ya no es un follow-up: `sooft-tech` reutilizaba, desde `sooft add`, un merge JSON idempotente con stamp `_sooftPack` (`merge.mjs`) que nunca estaba conectado a `agent install`; se conecta ahí (`standards.mjs` descubre `assets/hooks/adapters/*.json`, `agent.mjs` los mergea con esa misma función y los desinstala con `unmergeHooks`, nunca borrando el archivo de settings entero) y `sooft doctor` reporta el estado del hook por herramienta contra el filesystem real (no el manifest, porque el hook también puede llegar por el bootstrap en vivo de la skill, no solo por `agent install`). Modifica en `sooft-ai-standards`: `skills/sooft/SKILL.md` (§0.0, §0.1), `skills/sooft/internal/sooft-bootstrap.md` (PASO C.1 reescrito), `sooft-overview/index.html` y `sooft-overview/README.md`. Modifica en `sooft-tech`: `src/standards.mjs`, `src/constants.mjs`, `src/tools.mjs`, `src/commands/agent.mjs`, `src/commands/doctor.mjs`.

### Cambiado

- **PR y branching: se elimina un modelo de branching paralelo que nunca coincidió con SOOFT.** `assets/policies/git-guidelines.md` y `assets/reviews/compliance-review.md` describían una estrategia de branching (ramas `feat/`/`fix/`/`docs/`/`chore/` sin `security/` ni `migration/`, PR obligatoriamente contra `release/<version>` y nunca contra la rama principal, nombres de rama con el ticket embebido tipo `fix/INC-XXXXXX-descripcion-corta`) que **nunca fue la de SOOFT**: la real, ya implementada en los 4 routers (`git worktree add .worktrees/<tipo>-<slug> -b <tipo>/<slug>`), es worktree-first, sin rama de release intermedia, con el ticket viviendo en `.sooft/state.json.ticket` — nunca en el nombre de la rama. `sooft-validation.md` heredaba la misma contradicción al citar esos dos archivos como autoridad. Se reescriben las tres secciones para reflejar el modelo real, dejando explícito que un proyecto puede tener su propio modelo de integración (release branch u otro) y que SOOFT lo sigue **solo con evidencia**, nunca por defecto — mismo criterio que "no asumas el issue tracker sin evidencia". También se corrige un ejemplo de commit con un identificador de dominio bancario (`cvu-validation`, Clave Virtual Uniforme del sistema bancario argentino) que había quedado sin genericizar. Se genericizan además los prefijos de ticket tipo ITSM (`INC-`/`RITM-`/`STRY-`/`CHG-`/`REQ-`, típicos de ServiceNow) a la convención `TICKET-XXXXX` ya usada en el resto del repo, en 11 archivos: ambos `pull_request_template.md`, ambos `copilot-instructions.md`, `pr-template-guidelines.md`, `compliance-review.md`, `self-review-template.md`, `self-review-sketchpad-template.md`, `sooft-bugs/SKILL.md` y `sooft-validation.md`. Se agrega también la carpeta de artefactos de migración (`docs/migrations/{slug}/`) a la checklist de `compliance-review.md`, que solo cubría feat/bug/security.
