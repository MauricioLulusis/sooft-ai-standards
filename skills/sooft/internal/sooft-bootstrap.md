...
Instalar **una sola vez** los hooks de agente de Copilot, los custom agents de Copilot CLI y los archivos que la GUI de VS Code necesita (instrucciones globales + prompt files). SOOFT funciona también sin esto, pero el disparo determinista (banner + trigger de carga de la skill) lo dan los **hooks de agente de Copilot**; la ejecución especializada en Copilot CLI necesita los **custom agents** en `.github/agents/`; y la GUI de VS Code necesita además las **instrucciones globales** (`.github/copilot-instructions.md`) y los **prompt files** que habilitan los slash commands (`/sooft`, `/sooft-development`, `/sooft-bugs`, `/sooft-security-remediation`, `/sooft-status`, `/sooft-incident-response`). `npx skills add` baja la skill con sus assets, pero **no deja por sí solo** hooks, agents ni esos archivos en su lugar funcional. Por eso, **la primera vez que cargás la skill `sooft`** este auto-setup los instala una vez. Ejecutá este procedimiento EN ORDEN.
...
PASO D — Confirmá en UNA línea qué quedó instalado, sin fricción. NO le pidas al
         ...
         próxima sesión, de forma automática** (el harness de Copilot cachea los
...
> Qué hacen esos hooks, una vez instalados (los corre el harness de Copilot, NO el agente → no gastan tokens de razonamiento; valen igual en CLI y en la GUI de VS Code):
> - **`sessionStart`**: muestra el banner e inyecta el recordatorio de cargar `sooft` en cada sesión nueva.

> En **Claude Code** los hooks equivalentes se configuran por otro mecanismo (hooks de `settings.json`); no los instales acá.
